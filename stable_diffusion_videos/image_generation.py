...
def generate_images(
    ...
    if upsample:
        if getattr(pipeline, "upsampler", None) is None:
            pipeline.upsampler = upsampling.RealESRGANModel.from_pretrained("nateraw/real-esrgan")
    ...
    for batch_idx, prompt_batch in generate_input_batches([prompt] * num_images, batch_size_total):
        ...
        if jit:
            ...
            if len(prompt_batch) % NUM_TPU_CORES != 0:
                ...
                prompt_ids_batch = utils.pad_along_axis(prompt_ids_batch, pad_size, axis=0)
